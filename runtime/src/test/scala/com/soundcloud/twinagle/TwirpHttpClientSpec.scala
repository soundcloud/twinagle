...
class TwirpHttpClientSpec extends Specification {
  ...
  "error handling" >> {
    "redirect" in {
      ...
      val svc = Service.const(Future.value(response))
      val Throw(ex: TwinagleException) = Await.result(client(request, svc).liftToTry)
      ex.code ==== ErrorCode.Internal
      ex.meta should haveKeys("location", "http_error_code_from_intermediary", "status_code")
    }
    ...
    Fragments.foreach(Seq(
      ...
      case (status, errorCode) => s"HTTP ${status.code} produces $errorCode" ! {
        val response = Response(status)
        val svc = Service.const(Future.value(response))
        val Throw(ex: TwinagleException) = Await.result(client(request, svc).liftToTry)
        ex.code ==== errorCode
        ex.meta should haveKeys("body", "http_error_code_from_intermediary", "status_code")
      }
    }
